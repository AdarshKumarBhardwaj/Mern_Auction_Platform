...
const superAdminSlice = createSlice({
  ...
  reducers: {
    ...
    requestForDeletePaymentProofs(state, action) {
      state.loading = true;
    },
    successForDeletePaymentProofs(state, action) {
      state.loading = false;
    },
    failedForDeletePaymentProofs(state, action) {
      state.loading = false;
    },
...
export const getMonthlyRevenue = () => async (dispatch) => {
  ...
  try {
    ...
    // console.log("Response Data:", response.data); // Add this for debugging
    dispatch(
...
export const deletePaymentProof = (id) => async (dispatch) => {
  dispatch(superAdminSlice.actions.requestForDeletePaymentProofs());
  try {
    const response = await axios.delete(
      `http://localhost:5000/api/v1/superadmin/paymentproof/delete/${id}`,
      { withCredentials: true }
    );
    dispatch(superAdminSlice.actions.successForDeletePaymentProofs());
    dispatch(getAllPaymentProofs());
    toast.success(response.data.message);
  } catch (error) {
    dispatch(superAdminSlice.actions.failedForDeletePaymentProofs());
    console.error(error.response.data.message);
